...
Collection.prototype._initIndexes = function _initIndexes(definition, indexes) {
    //Get all index
    var indexNames = _.uniq(_.values(_.mapValues(definition, 'index')));

    indexNames.forEach(function (index) {
        if(index != undefined){
            var columns = _.pickBy(definition, function (attr) {
                return attr.index && attr.index.indexName === index.indexName;
            });
            var mappedIndex = {
            ...
            _.keys(columns).forEach(function (key) {
                ...
                if ((attr.hashKey || attr.rangeKey) &&
                    ...
                    if (attr.hashKey) {
                        ...
                        mappedIndex['hashKey'] = attr.localIndex ? this.schema.hashKey : key;
                    } else if (attr.index.rangeKey) {
                        mappedIndex['rangeKey'] = key;
                    }
                    if (attr.globalIndex) {
                        mappedIndex['type'] = 'global';
                    } else {
                        mappedIndex['type'] = 'local';
                    }
                }
            });
            // validate
            if (mappedIndex['type'] !== undefined && (mappedIndex['hashKey'] !== undefined ||
                mappedIndex['rangeKey'] !== undefined)) {
                mappedIndex['name'] = index.indexName;
    ...
    if (!query) {
        ...
        query.exec(function (err, data) {
            if (err) {
                ...
                cb(null, {items: items, count: data.Count, lastKey: data.LastEvaluatedKey});
            }
        });
    }
};
